...
public class LuaMgr : BaseManager<LuaMgr>
{
    ...
    private byte[] MyCustomLoader(ref string filepath)
    {
        ...
        string path = Application.dataPath + "/Res/lua/" + filepath + ".lua.txt";
        //C#自带的文件读取类
        if (File.Exists(path))
        {
    ...
    {
        //改为我们的AB包管理器加载
        TextAsset file2 = ResourcesComponent.GetInstance().GetAsset(filepath + ".lua", "Lua/"+ filepath+".lua")as TextAsset ;
        if (file2 != null)
            return file2.bytes;
        else
