...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderUnityEngineVector2
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterCaster<Tutorial.CSCallLua.DClass>(translator.Get);
			}
        }
		...
		public void Get(RealStatePtr L, int index, out Tutorial.CSCallLua.DClass val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    val = (Tutorial.CSCallLua.DClass)FastGetCSObj(L, index);
            }
			else if (type == LuaTypes.LUA_TTABLE)
			{
			    val = new Tutorial.CSCallLua.DClass();
				int top = LuaAPI.lua_gettop(L);
				
				if (Utils.LoadField(L, index, "f1"))
				{
					Get(L, top + 1, out val.f1);
				}
				LuaAPI.lua_pop(L, 1);
				
				if (Utils.LoadField(L, index, "f2"))
				{
					Get(L, top + 1, out val.f2);
				}
				LuaAPI.lua_pop(L, 1);
            ...
            {
                throw new Exception("can not cast " + LuaAPI.lua_type(L, index) + " to " + typeof(Tutorial.CSCallLua.DClass));
            }
        }
	...
	public partial class StaticLuaCallbacks
    {
		...
		internal static bool __tryArraySet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int array_idx, int obj_idx)
		{
			...
			else if (type == typeof(Tutorial.CSCallLua.DClass[]))
			{
			    Tutorial.CSCallLua.DClass[] array = obj as Tutorial.CSCallLua.DClass[];
				translator.Get(L, obj_idx, out array[array_idx]);
				return true;
			}
            return false;
